...
**Key behavior:** installs heavy workstation packages, installs and enables Docker Engine from Docker's official RHEL repository for local container validation, keeps npm and Python CLI tools under the dev user's home directory, installs repo-pinned Ansible collections when the checkout exists, and installs a `devmux` helper that attaches to or creates a persistent tmux session. Interactive SSH logins for the dev user show a short `devmux` usage hint, including `Ctrl-b`, then `d`, to detach from tmux. `vm2.yml` pairs this role with a VM2-only SELinux `domain_can_mmap_files` boolean for standard dev tooling.
